...
# Script principal pour exécuter les commandes make et sauvegarder les transactions en base de données
# Usage: ./run_commands.sh
...
save_to_database() {
    ...
    if PGPASSWORD="$DB_PASSWORD" psql -h "$DB_HOST" -p "$DB_PORT" -d "$DB_NAME" -U "$DB_USER" -c "$sql" > /dev/null 2>&1; then
        echo "✅ Données sauvegardées avec succès dans la table $table"
    else
        echo "❌ Erreur lors de la sauvegarde en base de données"
        echo "Vérifiez vos credentials et la connexion à la base de données"
        return 1
    fi
    echo ""
}
