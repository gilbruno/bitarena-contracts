...
# Script pour exécuter les commandes setMode et sauvegarder les transactions en base de données
# Usage: ./run_modes.sh
...
execute_make_command() {
    ...
    if output=$(eval "$command" 2>&1); then
        ...
        if [ -n "$tx_hash" ] && [ -n "$block_number" ]; then
            ...
            save_to_database "$mode_name" "$tx_hash" "$block_number"
        else
...
save_to_database() {
    ...
    INSERT INTO public.\"Mode\" (name, \"txHash\", \"blockNumber\")
    SELECT '$mode_name', '$tx_hash', $block_number
    ...
    if error_output=$(PGPASSWORD="$DB_PASSWORD" psql -h "$DB_HOST" -p "$DB_PORT" -d "$DB_NAME" -U "$DB_USER" -c "$sql" 2>&1); then
        echo "✅ Données sauvegardées avec succès dans la table Mode"
    else
...
execute_set_mode() {
    ...
    execute_make_command "$command" "$mode_name"
}
...
echo "⚙️  === Exécution des commandes setMode ==="
echo ""

# Exécuter les commandes setMode
execute_set_mode 2 1  # Créera un mode "2-1"
execute_set_mode 2 2  # Créera un mode "2-2"

echo "🏁 === Fin de l'exécution des commandes setMode ==="
echo "✅ Toutes les commandes setMode ont été exécutées et les données sauvegardées en base."
